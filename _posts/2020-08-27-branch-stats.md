...
**Note**: the original code had to be slightly modified to store the expression outcome in a separate variable (`_b`) before passing it to the inline assembly block. Otherwise, one of the tested systems GCC 10.2 complained about an invalid index expression.
